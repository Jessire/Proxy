...
if (url.includes("/x/resource/show/tab/v2")) {
  ...
  if (obj?.data?.ipad_more_sections?.length > 0) {
    const itemList = [798]; // 797我的客服 798设置 1070青少年守护
      ...
      if (item?.goto === "av") {
        ...
        if (item?.card_goto === "av" || item?.card_goto === "vertical_av") {
          if (item?.creative_entrance) {
            delete item.creative_entrance; // 话题推荐
          }
          if (item?.course_info) {
            delete item.course_info;
          }
          if (item?.free_flow_toast) {
            delete item.free_flow_toast; // 流量卡推荐
          }
          if (item?.game_info) {
          ...
          if (item?.scroll_guide) {
            delete item.scroll_guide; // 上滑观看提示
          }
